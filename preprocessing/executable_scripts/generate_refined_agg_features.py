...
def get_refined_agg_features(df_in : pd.DataFrame, num_agg_features : list,
    ...
    print('numeric aggs - last 5')
    df_in_num_agg_rec = get_agg_features(df_in=df_in.groupby('customer_ID').tail(5)
                                                    .add_suffix('_5sts').rename(columns={'customer_ID_5sts' : 'customer_ID'}), 
                                         group_col='customer_ID', 
                                         agg_features=[f'{f}_5sts' for f in num_agg_features], 
    ...
    df_in = pd.merge(df_in, df_in_num_agg_rec, how = 'inner', on = 'customer_ID')
    df_in = pd.merge(df_in, df_in_cat_agg, how = 'inner', on = 'customer_ID')

    del df_in_num_agg, df_in_num_agg_rec, df_in_cat_agg
    gc.collect()

    for c in num_agg_features:
        df_in[c + '_last_minus_mean'] = df_in[c + '_last'] - df_in[c + '_mean']
        df_in[c + '_last_minus_first'] = df_in[c + '_last'] - df_in[c + '_first']

        df_in[c + '_last_minus_5sts_mean'] = df_in[c + '_last'] - df_in[c + '_5sts_mean']
        df_in[c + '_5sts_mean_minus_mean'] = df_in[c + '_5sts_mean'] - df_in[c + '_mean']
...
df_train.to_parquet(CFG.output_dir + 'train_refined_agg_features.parquet')
...
print(df_test.shape)
df_test.to_parquet(CFG.output_dir + 'test_refined_agg_features.parquet')
