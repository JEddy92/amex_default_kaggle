...
def gen_sequence_data(df_in : pd.DataFrame, df_cat_enc : pd.DataFrame, 
    ...
    # df_in = pd.merge(df_in, df_nulls, on=['customer_ID'])
    ...
    # make days ago, missing statement, and months ago (position embedding) last features for convenience
...
df_test_nulls_chunk_1 = df_test_nulls[chunk_1_idx].reset_index(drop=True) # problem is index gets reset in merge, but order is preserved
